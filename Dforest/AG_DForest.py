...
class Algoritimo_genetico:
    ...
    def gerar_ind(self):
        ...
        min_sample_splits = uniform(0,1)
        n_estimators = randint(1,100)
        return [max_depth, criterion, min_sample_splits, n_estimators]
    ...
    def fitness(self, individuo):
        ...
        valor = individuo[1]
        floresta = RandomForestClassifier(max_depth= int(individuo[0]), criterion= criterion_dic[int(valor)], min_samples_split=individuo[2], n_estimators= individuo[3],random_state= 42)
    ...
    def crossover(self, selecao):
        ...
        if sorteio != 2:
            ...
            filho2.append(selecao[0][3])
            filho1.append(selecao[1][3])
    ...
    def mutar(self, nova_populacao):
        ...
        for i in range(len(nova_populacao)):
            ...
            chance = randint(1,100)
            if chance <= self.chance_m:
                    nova_populacao[i][2] = uniform(0,1)
            chance = randint(1,100)
            if chance <= self.chance_m:
                    nova_populacao[i][3] = randint(1, 100)
            populacao_mutada.append(nova_populacao[i])
        return populacao_mutada

    def finalizar(self):
        with open("/home/ismael/Documentos/PYTHON LEARNING PROJECTS/PYTHON_MACHINE_LEARNING/knn/evolucaoDforest.txt","w") as file:
